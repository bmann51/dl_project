...
class iBOTLoss(nn.Module):
    ...
    def __init__(self, out_dim, num_tokens, ncrops=8, 
                 ...
                 mim_temp=0.15, mim_temp_start=None, mim_temp_end=None,
                 use_focal_loss=False, focal_gamma=2.0, entropy_weight=0.1):
        super().__init__()
        ...
        self.mim_temp_start = mim_temp_start if mim_temp_start is not None else mim_temp
        self.mim_temp_end = mim_temp_end if mim_temp_end is not None else mim_temp
        ...
        # Temperature schedule for tokenizer (MIM)
        # Warmup from softer (higher temp) to sharper (lower temp) to prevent collapse
        # Softer early = prevents collapse, sharper later = maintains learning signal
        warmup_epochs = min(warmup_teacher_temp_epochs, nepochs // 2)  # Use same warmup as teacher
        if self.mim_temp_start != self.mim_temp_end:
            # Schedule: warmup from soft → sharp, then constant
            self.mim_temp_schedule = torch.cat((
                torch.linspace(self.mim_temp_start, self.mim_temp_end, warmup_epochs),
                torch.ones(nepochs - warmup_epochs) * self.mim_temp_end
            ))
        else:
            # Constant temperature (backward compatibility)
        ...
        if self.mim_loss_weight > 0:
            ...
            for crop_idx in range(self.ncrops):
                ...
                if masked_patches.sum() > 0:
                    ...
                    # Entropy regularization: encourage diverse predictions (prevents collapse)
                    # Higher entropy = more diverse token predictions = less collapse risk
                    # We subtract entropy from loss to encourage high entropy (diverse predictions)
                    if self.entropy_weight > 0:
                        # Entropy H = -sum(p * log(p)), student_log_probs already has log, so:
                        # student_probs * student_log_probs = p * log(p) (negative)
                        # -sum(...) = H (positive entropy)
                        entropy = -torch.sum(student_probs * student_log_probs, dim=-1).mean()
                        # Subtract entropy to encourage high entropy (diverse predictions)
                        mim_loss_crop = mim_loss_crop - self.entropy_weight * entropy
            ...
            if len(mim_losses) > 0:
                ...
                # Diagnostic: Detect MIM collapse (loss near zero after warmup)
                # MIM collapse happens when student predicts same token as teacher for all patches
                if mim_loss.item() < 0.001 and epoch > 5:
                    # This is a warning sign - model may have collapsed
                    # Store diagnostic info but don't spam logs
                    pass
            else:
                # No masked patches found - this shouldn't happen with mask_ratio > 0
