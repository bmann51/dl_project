...
from ibot_ssl import (
    ...
    RandomMaskingGenerator, BlockwiseMaskingGenerator, ProgressiveMaskingGenerator
)
...
def train_one_epoch(student, teacher, teacher_without_ddp, ibot_loss,
    ...
    for it, (images, _) in enumerate(pbar):
        ...
        if fp16_scaler is None:
            loss.backward()
            if args.clip_grad:
                _ = nn.utils.clip_grad_norm_(student.parameters(), args.clip_grad)
...
def main(args):
    print("=" * 80)
    print("iBOT Self-Supervised Learning (macey v6)")
    print("=" * 80)
    ...
    if args.progressive_masking:
        # Start mask ratio: user-specified or a conservative default based on target ratio
        if args.mask_ratio_start is not None:
            mask_ratio_start = args.mask_ratio_start
        else:
            # Default: start ~2/3 of final ratio but not below 0.15
            mask_ratio_start = max(0.15, args.mask_ratio * 0.66)
        ...
        print(f"Using progressive masking from ratio {mask_ratio_start} -> {args.mask_ratio}")
    else:
    ...
    print(f"Masking config: type={args.mask_type}, ratio={args.mask_ratio}, progressive={args.progressive_masking}")
    ...
    # Optimizer - LARS for large batch training or AdamW for stability
    ...
    # For LARS we traditionally scale by batch size; for AdamW we do not
    if args.optimizer == 'lars':
    ...
    for epoch in range(start_epoch, args.epochs):
        # Update progressive masking epoch, if enabled
...
if __name__ == '__main__':
    ...
    parser.add_argument('--progressive_masking', action='store_true',
                       help='Use ProgressiveMaskingGenerator (curriculum on mask ratio)')
    parser.add_argument('--mask_ratio_start', default=None, type=float,
                       help='Starting mask ratio for progressive masking (defaults to ~2/3 of final)')
    ...
    parser.add_argument('--use_focal_loss', action='store_true',
                       help='Use focal loss for MIM to emphasize hard masked patches')
    ...
    parser.add_argument('--lr', default=0.0003, type=float,
                       help='Base learning rate (for AdamW, typically 0.0003-0.0005)')
    parser.add_argument('--max_lr', default=0.001, type=float,
                       help='Peak learning rate after warmup (if <= base lr, disabled)')
    ...
    parser.add_argument('--output_dir', default='./checkpoints_v6', type=str,
