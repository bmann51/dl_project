...
class RandomMaskingGenerator:
    ...
    Masks patches in a random per-patch manner.
    """
...
if __name__ == "__main__":
    # Quick test
    print("Testing iBOT components (v6, macey)...")
    ...
    prog_mask_gen = ProgressiveMaskingGenerator(96, mask_ratio_start=0.2, mask_ratio_end=0.4, total_epochs=100)
    prog_mask_gen.set_epoch(0)
    mask0 = prog_mask_gen()
    prog_mask_gen.set_epoch(99)
    mask99 = prog_mask_gen()
    print(f"Progressive mask (epoch 0) masked patches: {(mask0 == 0).sum()}")
    print(f"Progressive mask (epoch 99) masked patches: {(mask99 == 0).sum()}")
    
    print("\nAll v6 component tests passed!")
