"""
Submission script for iBOT trained models (macey v6)

Usage:
    python generate_submission.py \
        --checkpoint /gpfs/scratch/bm3772/checkpoints/v6/checkpoints_ibot_v6_liberal_6/final_checkpoint.pth \
...
class iBOTHead(nn.Module):
    """Projection head for iBOT - matches training architecture (NO BatchNorm for v6)"""
    def __init__(self, in_dim, out_dim=65536, hidden_dim=2048, bottleneck_dim=256, 
        ...
        # Build MLP - v6 training uses NO BatchNorm, just Linear + GELU
        layers = []
...
def load_ibot_model(checkpoint_path, arch='vit_small', img_size=96, 
                    out_dim=4096, bottleneck_dim=256, num_tokens=8192, device='cuda'):
    ...
    # Create iBOT heads - v6 uses NO BatchNorm (matches training code)
