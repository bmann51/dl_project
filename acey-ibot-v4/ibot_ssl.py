...
class CNNFeatureExtractor(nn.Module):
    """
    Extracts spatial features from CNN feature maps and converts to patch-like tokens.
    For iBOT, we need patch-level features for masking, so we extract spatial features
    from CNN feature maps and treat them as "patches".
    """
    def __init__(self, backbone, target_spatial_size=6, embed_dim=512):
        """
        Args:
            backbone: CNN backbone (e.g., ResNet)
            target_spatial_size: Target spatial size (e.g., 6x6 for 96x96 images with 16x16 patches)
        ...
        # For ResNet, typically 512 (ResNet18/34) or 2048 (ResNet50)
        with torch.no_grad():
            dummy_input = torch.randn(1, 3, 96, 96)
            try:
                # Try forward_features first (timm standard)
                if hasattr(self.backbone, 'forward_features'):
                    features = self.backbone.forward_features(dummy_input)
                else:
                    # Fallback to forward without classification head
                    features = self.backbone.forward(dummy_input)
                
                if isinstance(features, torch.Tensor):
                    # Feature map: [B, C, H, W] or [B, C] (if global pooled)
                    if len(features.shape) == 4:
                        feat_dim = features.shape[1]
                        spatial_h, spatial_w = features.shape[2], features.shape[3]
                    elif len(features.shape) == 2:
                        # Already global pooled, use adaptive pooling
                        feat_dim = features.shape[1]
                    ...
                    # If backbone returns tuple or dict, use first element
                    feat_dim = features[0].shape[1] if isinstance(features, (tuple, list)) else 512
                    spatial_h, spatial_w = 1, 1
            except Exception as e:
                # Fallback to defaults
                print(f"Warning: Could not infer feature dim from backbone, using defaults: {e}")
                feat_dim = 512
                spatial_h, spatial_w = 1, 1
        ...
        # Projection to target embed_dim
        self.proj = nn.Linear(feat_dim, embed_dim)
        
        # Learnable CLS token (like ViT)
    ...
    def forward(self, x):
        ...
        Returns:
            features: [batch_size, num_patches + 1, embed_dim] (patches + CLS token)
        """
        # Extract features from CNN
        if hasattr(self.backbone, 'forward_features'):
            feat_map = self.backbone.forward_features(x)  # [B, C, H, W] or [B, C]
        else:
            feat_map = self.backbone.forward(x)  # Fallback
        
        # Handle different output shapes
        if len(feat_map.shape) == 2:
            # Already global pooled [B, C], need to reshape for spatial features
            B, C = feat_map.shape
            # Reshape to spatial and use adaptive pooling
            feat_map = feat_map.unsqueeze(-1).unsqueeze(-1)  # [B, C, 1, 1]
            feat_map = self.adaptive_pool(feat_map)  # [B, C, target_h, target_w]
        elif len(feat_map.shape) == 4:
            # Feature map [B, C, H, W]
            feat_map = self.adaptive_pool(feat_map)  # [B, C, target_h, target_w]
        else:
            raise ValueError(f"Unexpected feature map shape: {feat_map.shape}")
        
        # Flatten spatial dimensions
        B, C, H, W = feat_map.shape
        feat_map = feat_map.permute(0, 2, 3, 1)  # [B, H, W, C]
        feat_map = feat_map.reshape(B, H * W, C)  # [B, num_patches, C]
        
        # Project to target embed_dim
        feat_map = self.proj(feat_map)  # [B, num_patches, embed_dim]
        
        # Add CLS token
        cls_tokens = self.cls_token.expand(B, -1, -1)  # [B, 1, embed_dim]
        features = torch.cat([cls_tokens, feat_map], dim=1)  # [B, num_patches + 1, embed_dim]
...
def get_backbone(arch='vit_small', patch_size=16, img_size=96, drop_path_rate=0.0):
    """
    Create a backbone network. Supports both ViT and CNN architectures.
    
    For CNN architectures, returns a CNNFeatureExtractor wrapper that converts
    CNN feature maps to patch-like tokens compatible with iBOT.
    """
    # Calculate target spatial size (for CNN feature extraction)
    # For 96x96 images with 16x16 patches: 6x6 patches
    target_spatial_size = img_size // patch_size
    ...
    if 'vit' in arch:
        ...
        if arch == 'resnet18':
            backbone = timm.create_model('resnet18', pretrained=False, num_classes=0, global_pool='')
            embed_dim = 512  # ResNet18/34 feature dim
        elif arch == 'resnet34':
            backbone = timm.create_model('resnet34', pretrained=False, num_classes=0, global_pool='')
            embed_dim = 512
        elif arch == 'resnet50':
            backbone = timm.create_model('resnet50', pretrained=False, num_classes=0, global_pool='')
            embed_dim = 2048  # ResNet50+ feature dim
        else:
            raise ValueError(f"Unknown ResNet architecture: {arch}")
        
        # Wrap with feature extractor to convert to patch-like tokens
        model = CNNFeatureExtractor(backbone, target_spatial_size=target_spatial_size, embed_dim=embed_dim)
        return model, embed_dim
        ...
        if arch == 'efficientnet_b0':
            ...
            backbone = timm.create_model('efficientnet_b1', pretrained=False, num_classes=0, global_pool='')
            embed_dim = 1280
        else:
        ...
        if arch == 'convnext_tiny':
            backbone = timm.create_model('convnext_tiny', pretrained=False, num_classes=0, global_pool='')
            embed_dim = 768
        elif arch == 'convnext_small':
            backbone = timm.create_model('convnext_small', pretrained=False, num_classes=0, global_pool='')
            embed_dim = 768
        else:
            raise ValueError(f"Unknown ConvNeXt architecture: {arch}")
        
        model = CNNFeatureExtractor(backbone, target_spatial_size=target_spatial_size, embed_dim=embed_dim)
        return model, embed_dim
    
    else:
        raise ValueError(f"Unknown architecture: {arch}. Supported: vit_*, resnet_*, efficientnet_*, convnext_*")
