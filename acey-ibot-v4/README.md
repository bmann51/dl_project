# iBOT Implementation (v4 - Architecture Experiments)

This folder contains an iBOT (Image BERT Pre-Training with Online Tokenizer) implementation for self-supervised learning, with **CNN backbone support** and **architectural experiments** to explore which backbone works best.
...
### Architecture Experiments (NEW in v4)
1. **CNN Backbone Support**: ResNet, EfficientNet, ConvNeXt architectures
2. **CNN Feature Extractor**: Converts CNN feature maps to patch-like tokens for iBOT
3. **Multiple Architecture Options**: Compare ViT vs CNN vs Hybrid architectures

### Architectural Enhancements (from v3)
...
See `TRAINING_VARIANTS.md` for detailed comparison and experiment strategy.
...
iBOT uses a **teacher-student** framework with **multiple backbone options**:
...
Input Image (96x96)
    ↓
[CNN Backbone] → Feature maps [batch, C, H, W]
    ↓
[CNNFeatureExtractor] → Spatial features [batch, 37, embed_dim] (36 patches + 1 CLS token)
    ↓
    ├─→ [CLS Token] → [iBOTHead] → Global features (for self-distillation)
    └─→ [All Tokens] → [iBOTTokenizer] → Token predictions (for MIM)
```
...
- `ibot_ssl.py`: Core iBOT implementation
  ...
  - `CNNFeatureExtractor`: **NEW in v4** - Converts CNN feature maps to patch-like tokens
  ...
  - `get_backbone`: **ENHANCED in v4** - Supports ViT, ResNet, EfficientNet, ConvNeXt architectures
...
- **Backbone**: ViT (tiny/small/base) or CNN (ResNet/EfficientNet/ConvNeXt)
- **Feature Extractor**: For CNN, converts feature maps to patch-like tokens
- **Tokenizer**: MLP mapping patch embeddings to discrete tokens
- **Head**: Projection head for CLS token (similar to DINO)

See `ARCHITECTURE_EXPERIMENTS.md` for details on architecture experiments.
...
- **ResNet18 backbone**: ~11M parameters ✅ (well under 100M limit)
- **ResNet34 backbone**: ~21M parameters ✅ (well under 100M limit)
- **ResNet50 backbone**: ~25M parameters ✅ (well under 100M limit)
- **EfficientNet-B0 backbone**: ~5M parameters ✅ (well under 100M limit)
- **EfficientNet-B1 backbone**: ~8M parameters ✅ (well under 100M limit)
- **ConvNeXt-Tiny backbone**: ~28M parameters ✅ (well under 100M limit)
- **ConvNeXt-Small backbone**: ~50M parameters ✅ (well under 100M limit)
...
| **ViT-Tiny** | ~5.5M | ✅ **SAFE** |
| **ViT-Small** | ~22M | ✅ **SAFE** |
| **ViT-Base** | ~86M | ✅ **SAFE** (close to limit) |
| **ResNet18** | ~11M | ✅ **SAFE** |
| **ResNet34** | ~21M | ✅ **SAFE** |
| **ResNet50** | ~25M | ✅ **SAFE** |
| **EfficientNet-B0** | ~5M | ✅ **SAFE** |
| **EfficientNet-B1** | ~8M | ✅ **SAFE** |
| **ConvNeXt-Tiny** | ~28M | ✅ **SAFE** |
| **ConvNeXt-Small** | ~50M | ✅ **SAFE** |
...
**Architecture experiments (NEW in v4):**
- **CNN backbone support**: ResNet, EfficientNet, ConvNeXt architectures
- **CNNFeatureExtractor**: Converts CNN feature maps to patch-like tokens
- **Multiple architecture options**: Compare ViT vs CNN for iBOT
- **Architecture experiment scripts**: ResNet18 and ConvNeXt-Tiny training scripts

**Architectural improvements (from v3):**
...
```bash
# ResNet18 (classic CNN)
python train_ibot.py --arch resnet18 [other args...]

# ConvNeXt-Tiny (modern CNN)
python train_ibot.py --arch convnext_tiny [other args...]

# EfficientNet-B0 (efficient CNN)
python train_ibot.py --arch efficientnet_b0 [other args...]
```

### How CNN Works with iBOT

CNNs output feature maps, not patches. The `CNNFeatureExtractor` automatically:
...
This makes CNN features compatible with existing iBOT masking and tokenization - no changes needed to training code!

### Architecture Comparison

See `TRAINING_VARIANTS.md` for detailed comparison of ResNet vs ConvNeXt vs ViT architectures.
