...
def train_one_epoch(student, teacher, teacher_without_ddp, ibot_loss,
    ...
    for it, (images, _) in enumerate(pbar):
        ...
        # Generate mask for student views (same patch grid, independent per image)
        ...
        with autocast_context(fp16_scaler is not None):
            ...
            # Student: same images, loss computed on masked patches
        ...
        if (epoch % 5 == 0 and it == 0) or (it % 100 == 0):
            print(f"Loss components: mim={loss_components['mim_loss']:.4f}, "
                  ...
                  f"k o l e o={loss_components['koleo_loss']:.4f}")
...
if __name__ == '__main__':
    ...
    # Loss weights
    parser.add_argument('--mim_loss_weight', default=0.5, type=float,
                        help='Weight for MIM loss')
    parser.add_argument('--cls_loss_weight', default=1.5, type=float,
    ...
    parser.add_argument('--mim_temp', default=0.15, type=float,
                        help='MIM tokenizer temperature')
    ...
    # Evaluation pooling
    parser.add_argument('--eval_pool', default='mean', type=str,
                        choices=['cls', 'mean'],
                        help='How to pool tokens for evaluation (single image forward)')
