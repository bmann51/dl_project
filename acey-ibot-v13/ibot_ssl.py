...
class MultiCropiBOTWrapper(nn.Module):
    ...
    Teacher sees full images, student sees multiple crops.
    For evaluation (single image): returns pooled, normalized features.
    """
    def __init__(self, backbone, head, tokenizer,
        ...
        self.eval_pool = eval_pool  # 'cls' or 'mean'
    ...
    def forward(self, x, mask=None, return_patch_tokens=False):
        ...
        Returns:
            If return_patch_tokens=False:
                head_output: Projection head output (CLS token outputs for all crops)
            If return_patch_tokens=True:
                (head_output, token_logits): Both head output and token predictions
                token_logits: [total_crops * batch, num_patches+1, num_tokens]
        """
        # Handle list of crops (training)
        if isinstance(x, list):
            ...
            if return_patch_tokens:
                return (
                    torch.cat(outputs) if outputs else torch.empty(0),
                    torch.cat(token_outputs) if token_outputs else torch.empty(0)
                )
            return torch.cat(outputs) if outputs else torch.empty(0)

        # Single image (evaluation / feature extraction)
        features = self.backbone(x)  # [B, 1+N, dim]
        if self.eval_pool == 'mean':
            # Mean over patch tokens only
            tokens = features[:, 1:, :]      # [B, N, dim]
            feat = tokens.mean(dim=1)        # [B, dim]
        else:
            # CLS token
            feat = features[:, 0]            # [B, dim]
...
class iBOTLoss(nn.Module):
    ...
    2. Self-distillation loss: Consistency on visible patches (CLS)
    3. KoLeo regularization: Feature diversity
    """
    ...
    def forward(self, student_output, teacher_output,
        ...
        if self.mim_loss_weight > 0:
            ...
            for crop_idx in range(self.ncrops):
                ...
                # Student tokens for this crop (exclude CLS)
                student_tokens_crop = student_token_logits[start_idx:end_idx, 1:, :]
                # Teacher tokens (first global view)
                teacher_tokens_crop = teacher_token_logits[:batch_size, 1:, :]

                masked_patches = (student_mask == 0)  # [batch, num_patches]
                num_masked = masked_patches.sum().item()

                if num_masked > 0:
                    teacher_tokens_masked = teacher_tokens_crop[masked_patches]
                    teacher_tokens_masked = F.softmax(
                        teacher_tokens_masked / self.mim_temp, dim=-1
                    ).detach()

                    student_tokens_masked = student_tokens_crop[masked_patches]
    ...
    def compute_koleo_loss(self, features):
        ...
        KoLeo loss = -mean(log(pairwise_distances + eps))
        """
        features = F.normalize(features, dim=-1, p=2)
        pairwise_dist = torch.cdist(features, features, p=2) ** 2
...
class DataAugmentation:
    """
    Multi-crop augmentation for iBOT (similar to DINO), tuned to keep fine details
    while still providing strong invariances.
    """
    def __init__(self, global_crops_scale=(0.4, 1.), local_crops_scale=(0.05, 0.4),
                 local_crops_number=6, size=96):
        # Global crops (2) - strong but not insane
        self.global_transfo1 = transforms.Compose([
            ...
            transforms.RandomErasing(p=0.075, scale=(0.02, 0.2)),
        ])
...
def cosine_scheduler_with_peak(peak_value, final_value, epochs, niter_per_ep,
    ...
    """
    Cosine schedule with warmup to a peak, then cosine decay.
    """
...
if __name__ == "__main__":
    ...
    # Test masking
    mask_gen = RandomMaskingGenerator(96, mask_ratio=0.3)
    mask = mask_gen()
